...
const RELEASE_NOTES: ReleaseNote[] = [
    {
        ...
        date: '2025-11-03',
        showOnUpdate: true,
        info: 'This feature has three main improvements: First: ==multi-tag selection==. If you love working with tasks this will improve your productivity a lot. Secondly: ==much better in vertical split mode. You can now choose to hide pinned files header, you can reduce the line spacing in slim file mode, and the current folder name is not shown if you group by subfolders. You can now also override group by for every folder individually. Thirdly: ==much better background rendering in navpane==. This is huge if you use inherit folder colors and use different background colors for each root folder. A new setting **Root folder spacing** allows you to separate root items further, making for a very pleasing visual setup. Enjoy!',
        new: [
