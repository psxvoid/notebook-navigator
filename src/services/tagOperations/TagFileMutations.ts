...
export class TagFileMutations {
        ...
        try {
            await this.app.fileManager.processFrontMatter(file, fm => {
                ...
                if (typeof fm.tags === 'string') {
                    const tags = fm.tags.split(',').map((value: string) => value.trim());
                    tags.push(tag);
    ...
    private async stripInlineTags(file: TFile, tags: string[]): Promise<boolean> {
        ...
        if (typeof cachedContent === 'string') {
            ...
            for (const { pattern } of tagPatterns) {
                const nextContent = this.removeInlineTagsWithPattern(updatedContent, pattern);
        ...
        await this.app.vault.process(file, content => {
            ...
            for (const { pattern } of tagPatterns) {
                const updated = this.removeInlineTagsWithPattern(nextContent, pattern);
    ...
    private removeInlineTagsWithPattern(content: string, pattern: RegExp): string {
        pattern.lastIndex = 0;
