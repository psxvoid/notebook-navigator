...
export const STRINGS_JA = {
    ...
    settings: {
        ...
        items: {
            ...
            tagProps: {
                tagPropMode: {
                    name: 'タグモード（ボールトプロファイル）',
                    desc: 'このVaultプロファイルで使用され、タグツリーに表示されるタグモード。「なし」は通常のタグが使用されていることを意味します。「置換」は、カスタムタグプロパティのみが表示され、通常のタグが非表示になることを意味します。「マージ」は、タグプロパティと通常のタグの両方が表示されることを意味します。',
                    options: {
                        none: 'なし',
                        replace: '交換する',
                        merge: 'マージ',
                ...
                mainTagProp: {
                    name: 'メインタグ（ボールトプロファイル）',
                    desc: 'どのタグまたはタグプロパティを「メイン」タグとして扱うかを指定できます。メインタグはタグ編集操作で使用されます。「デフォルト」は、通常のタグがメインタグとして設定されることを意味します。「最初のタグプロパティ」は、「タグプロパティリスト」の最初のタグプロパティが「メイン」として設定されることを意味します。',
                    options: {
                        default: 'デフォルト',
                        firstTagProp: '最初のタグプロパティ',
                    }
                },
                tagPropList: {
                    name: 'タグのプロパティ（ボールトプロファイル）',
                    desc: 'フロントマタープロパティのコンマ区切りリスト。リストされたプロパティは、タグ（例：base-tags、note-class-tags）の追加ソースとして扱われます。',
                    placeholder: 'ベースタグ、カスタムタグ',
                },
            },
            ...
            featureImageForPDF: {
                name: 'PDFの注目画像',
                desc: 'PDF の注目画像を生成できます。'
            },
            featureImagePersistIntermediate: {
                name: 'フル機能の画像をディスクに保存する',
                desc: '上級ユーザー向け。このオプションを有効にすると、フルフィーチャーの中間画像がディスクに保存されます。フィーチャー画像のサイズを変更したり同期したりする際の初期インデックス作成を高速化するために役立つ場合があります。有効にするとキャッシュの再構築が必要になります。無効にすると、中間画像はクリーンアップされません。'
            },
            showRootFolder: {
                name: 'ルートフォルダを表示',
            ...
            rebuildCacheFast: {
                name: 'キャッシュを更新',
                desc: `タグの欠落、プレビューの誤り、または注目画像の欠落が発生した場合は、こちらをご利用ください。同期の競合や予期せぬ終了によって発生する可能性があります。「キャッシュの再構築」よりも軽量版ですが、すべての問題が解決するとは限りません。`,
                buttonText: 'キャッシュを更新',
                success: 'キャッシュが更新されました',
